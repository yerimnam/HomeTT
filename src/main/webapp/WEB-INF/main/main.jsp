<%@page import="party.dto.Party"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
    <%List<Party> partyList = (List)request.getAttribute("mainlist"); %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>HomeTT</title>
<script type="text/javascript" src="https://code.jquery.com/jquery-2.2.4.min.js" ></script>
<script type="text/javascript">


</script>

<style type="text/css">

@import url('https://webfontworld.github.io/sunn/SUIT.css');
html, body, pre, h1, h2, h3, h4, h5, h6, dl, dt, dd, ul, li, ol, th, td, p, blockquote, form, fieldset, legend, menu, nav, section, hgroup, article, header, aside, footer, input, select, textarea, button {
    margin: 0;
    padding: 0;
    font-family: 'SUIT';
}

header, footer{

	height: 300px;

}
body{
		width: 1600px;
		margin: 0 auto;
	
	}

	
ul{
	list-style: none;
	text-align: center;
}

#contents_area{
    height: 423px;
    position: relative;
}
.recommend_contents{

    border: 2px solid #666666;
    position: absolute;
    top: 157px;
    left: 207px;
    height: 300px;
       top: 74px;
    left: 165px;
    height: 300px;

}

 .contents {

     float: left; 
    position: relative;
    left: 7px;
    top: 14px;
    margin-right: 35px;
    width: 200px;
    height: 200px;
    margin-left: 20px;
}

img{
 	width: 200px; 
	height: 230px;
	 overflow:hidden;
	transition: scale(1)


}
.contents:hover img{
	transform : scale(1.2,1.2);
	transition-duration: 0.3s;

}

#homett{

 	top: -16px;
    position: relative;
    width: 1216px;
    left: 4px;
    color: #ffde59;
    font-weight: bold;
    font-size: 33px;

  

}

#homett_area{
   position: relative;
    top: 64px;
    left: 166px;
    width: 1213px;

}
a{

	text-decoration: none;
}

a:visited {

	color:black;
}


.content_title:hover{
	color: #ffde59
}
.content_title{

	color: #666666;
      paddingg-top: 10px;
    float: left;
    position: relative;
    text-align: center;
    font-size: 20px;
    padding-left: 20px];
    padding-top: 20px;
    left: 76px;

}



.move{left: 60px;}
.move_02 {left: 70px;}


#line{
	top: 236px;
    width: 1218px;
    border: 1px solid black;
    position: relative;
    top: 300;
    left: 10px;

}





#popular_ott{
    position: relative;
    top: 100px;

}

.ottname{

	float:left;
    border: 1px solid black;
    margin: 0 50px;
    left: 164px;
    position: relative;
    width: 200px;
    height: 200px;
	

}
#popular_ott ul {

	float:left;
	position: absolute;
}

#ott_area{
    position: relative;
    top: 20px;
    left: 166px;
}

#ott{

    position: absolute;
    font-size: 33px;
    color: #ffde59;
    font-weight: 700;

}
</style>

</head>
<body>


<header>
헤더 영역

</header>
<hr>


<div id="homett_area">
	<span id="homett">HomeTT 추천 </span>
</div>
<div id="contents_area" >


	<ul class="recommend_contents">

		<li class="contents"><a href="http://program.tving.com/tvn/queensumbrella">
			<img src="http://img.lifestyler.co.kr/uploads/program/seditor/2022/09/f637977178222536498.jpg">
			<span class="content_title 01">슈룹</span>
			</a>
			</li>
		
		<li class="contents"><a href="https://www.tving.com/contents/P001613931?utm_source=Naver&utm_medium=Organic&utm_campaign=SERP">
			<img src="http://img.lifestyler.co.kr/uploads/program/seditor/2022/08/f637962481211509993.jpg">
			<span  class="content_title move">작은아씨들</span>
			</a>
		</li>
		
		<li class="contents"><a href="https://movie.naver.com/movie/bi/mi/basic.naver?code=190695">
			<img src="https://search.pstatic.net/common?type=o&size=174x242&quality=75&direct=true&src=https%3A%2F%2Fcsearch-phinf.pstatic.net%2F20220923_135%2F1663929089524ezlHE_JPEG%2Fcc3aa8184053d216ff475cb583174da1.jpg">
			<span class="content_title 03">환승연애</span>
			</a>
		</li>
		
		<li class="contents"><a href="https://www.netflix.com/title/81343748?source=naver">
			<img src=" https://search.pstatic.net/common?type=o&size=174x242&quality=75&direct=true&src=https%3A%2F%2Fcsearch-phinf.pstatic.net%2F20220902_274%2F1662110955038PohIe_JPEG%2F8d0822671c062180977ecbf54af88edb.jpg">
			<span class="content_title 04">수리남</span>
			</a>
		</li>
		<li class="contents"> <a href="https://www.youtube.com/watch?v=T3_RgSypJBw"> 
			<img src="https://search.pstatic.net/common?type=f&size=138x200&quality=75&direct=true&src=https%3A%2F%2Fs.pstatic.net%2Fmovie.phinf%2F20220405_94%2F1649134635502Ko6SS_JPEG%2Fmovie_image.jpg%3Ftype%3Dw640_2">
			<span class="content_title 05 move_02"> 모비우스</span>
			</a>
			</li>
	
	</ul>
</div>
<div id="ott_area">
	<span id="ott"> 인기 OTT</span>
</div>

<div id="popular_ott">



	<ul>
		<li class="ottname netflix"><img src = "https://images.ctfassets.net/4cd45et68cgf/Rx83JoRDMkYNlMC9MKzcB/2b14d5a59fc3937afd3f03191e19502d/Netflix-Symbol.png?w=684&h=456"></li>
		<li class="ottname watcha"><img src= "data:image/svg+xml;base64,"></li>
		<li class="ottname wavve">웨이브</li>
		<li class="ottname tving">TVING</li>
	</ul>

</div>

<!--   <div id="partylist">  --> 
<!--  	<ul>   -->
	
<%-- 	<% for(int i=0; i<partyList.size(); i++) { %> --%>
<%-- 		<li><%=partyList.get(i).getPartyKind() %></li> --%>
<%-- 		<li><%=partyList.get(i).getPartyName() %></li> --%>
<%-- 		<li><span>파티장 : </span> <span><%=partyList.get(i).getPartyLeader() %></span> --%>
<%-- 		<li><span>파티만료일 : </span><%=partyList.get(i).getPartyEndDate() %></li> --%>
<%-- 		<li><span>남은 파티 기간 : </span><%=partyList.get(i).getPartyPeriod() %></li> --%>
<%-- 		<li><span>모집인원 : </span><%=partyList.get(i).getPartyMember() %>명</li> --%>
<%-- 		<li><span>참여 금액 : </span> <%=partyList.get(i).getPaymentAmount() %>원</li> --%>
<%-- 	<%} %> --%>
	
	
<!-- 	</ul>  -->




<!-- </div> -->

<!-- <footer>  -->

<!--  푸터 영역  -->

<!--  </footer>  -->

</body>
</html>